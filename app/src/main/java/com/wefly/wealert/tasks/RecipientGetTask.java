...
public class RecipientGetTask extends AsyncTask<List<Recipient>, Integer, List<Recipient>> {
    ...
    @Override
    protected List<Recipient> doInBackground(List<Recipient>... lists) {
    ...
    private List<Recipient> extractRecipients(@NonNull String json) {
        ...
        try {
            ...
            for (int i = 0; i < response_recipients.length(); i++) {
                ...
                recipient.setUserName(user.getString("username"));
